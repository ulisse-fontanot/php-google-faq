    ...
    [
        ...
        'risposte' => [
            'parte-sopra' => [
                ...
                'elenco-numerato' => [
                    'uno' => [
                        'titolo' => "La società consociata Google che offre i servizi, tratta le tue informazioni ed è responsabile del rispetto delle leggi sulla privacy vigenti. Generalmente Google offre i propri servizi per i consumatori tramite una delle due società seguenti:",
                        'elenco-alfa' => [
            ...
            'parte-sotto' => [
                ...
                'testo' => "Quando crei un nuovo Account Google, lo associamo a un paese in base a dove è stato creato. Per quanto riguarda gli account creati almeno un anno fa, usiamo il paese da cui accedi solitamente ai servizi Google, in genere i servizi in cui hai trascorso più tempo nell'ultimo anno.
...
<div class='container'>
    ...
    <main>
        <?php foreach ($array as $sezioni) { ?>
            <?php foreach ($sezioni as $key => $sezione) { ?>
                <?php if ($key == 'domanda') { ?>
                  <!-- STAMPA DOMANDA -->
                    <h2><?php echo $sezioni['domanda']?></h2>
                <?php } elseif ($key == 'risposta') { ?>
                  <!-- STAMPA RISPOSTA -->
                    <p><?php  echo $sezioni['risposta'] ?></p>
                <?php } else {?>
                  <!-- STAMPA RISPOSTA ARTICOLATA (TERZA SEZIONE) -->
                    <?php foreach ($sezione as $key2 => $parte) { ?>
                        <!-- PARTE SOPRA -->
                        <?php if ($key2 == 'parte-sopra') { ?>
                            <p><?php echo $parte[0]?></p>
                            <ol>
                                <?php foreach ($array[2]['risposte']['parte-sopra']['elenco-numerato'] as $key4 => $elNum) { ?>
                                    <?php if ($key4 == 'uno') { ?>
                                        <li><?php foreach ($elNum as $key5 => $elUno) { ?>
                                            <?php if ($key5 == 'titolo') { ?>
                                                <h4><?php echo $elUno?></h4>
                                            <?php } else {?>
                                                <ol>
                                                    <?php foreach ($array[2]['risposte']['parte-sopra']['elenco-numerato']['uno']['elenco-alfa'] as $elAlfa) { ?>
                                                        <li><?php echo $elAlfa ?></li>
                                                    <?php } ?>
                                                </ol>
                                            <?php } ?>
                                        <?php } ?></li>
                                    <?php } else {?>
                                        <li><?php echo $elNum ?></li>
                                    <?php } ?>
                                <?php } ?>
                            </ol>
                            <p><?php echo $array[2]['risposte']['parte-sopra'][2]?></p>
                        <!-- PARTE SOPRA -->
                        <!-- PARTE SOTTO -->
                        <?php } else {?>
                            <?php foreach ($parte as $key3 => $sotto) { ?>
                                <?php if ($key3 == 'titolo') { ?>
                                    <!-- TITOLO -->
                                    <h3><?php echo $array[2]['risposte']['parte-sotto']['titolo']?></h3>
                                <?php } else {?>
                                    <!-- TESTO -->
                                    <p><?php echo $array[2]['risposte']['parte-sotto']['testo']?></p>
                                <?php } ?>
                            <?php } ?>
                        <?php } ?>
                        <!-- PARTE SOTTO -->
